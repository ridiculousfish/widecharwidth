...
OUTPUT_FILENAME_RUST = "widechar_width.rs"
...
if __name__ == "__main__":
    ...
    fields = make_fields(datas, cps, LangSettings("{}"), OUTPUT_FILENAME)
    with open(OUTPUT_FILENAME, "w") as fd:
        fd.write(OUTPUT_TEMPLATE.strip().format(**fields))
        fd.write("\n")
    log("Output " + OUTPUT_FILENAME)

    fields = make_fields(datas, cps, LangSettings("[]"), OUTPUT_FILENAME_JS)
    with open(OUTPUT_FILENAME_JS, "w") as fd:
        fd.write(OUTPUT_TEMPLATE_JS.strip().format(**fields))
        fd.write("\n")
    log("Output " + OUTPUT_FILENAME_JS)

    fields = make_fields(datas, cps, LangSettings("()"), OUTPUT_FILENAME_RUST)
    with open(OUTPUT_FILENAME_RUST, "w") as fd:
